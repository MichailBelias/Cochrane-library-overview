...
* in Practical 4a as the IPD you used was based on that for trial 1 here.
...
* There are many suggested approaches for dealing with missing within-study 
...
* ourselves as in Practical 4(a), and so avoid the issue of missing 
...
* We can also view the amount of direct evidence and the network map

network pattern
network map


* taken from the help file, us this to get a plot with non-overlapping lines *
mat loc3=( 3,3,9\ 5,4,3\ 1,2,9\ 4.33,3,6\ 6,4.5,3\ 3,3.67,9\ 5,2,3\ 1,4,9)
network map, loc(loc3) 

* Let us perform a network meta-analysis
* We use treatment A as the reference treatment, and therefore we have
...
* Q7: All the results are relative to the reference treatment A
* Therefore, if I do the following, what am I now estimating?
lincom [_y_B]_cons - [_y_C]_cons

* The summary results can be presented on a network forest plot:
...
* We can also rank treatments, and examine the probability that they are
* best, 2nd best, 3rd best, and so on.
* Probability of each ranking
network rank min, line meanrank 
* Cumulative probability across ranking (SUCRA)
network rank min, line meanrank cumulative

* As mention in the lecture, treatment G has highest probability of being 1st
