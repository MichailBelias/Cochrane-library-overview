...
* The metareg command wants the standard errors after the comma, unlike metan.
...
* Note that due to lack of convergence we still do not allow for separate
...
* interaction can explain within-study and between-study variation. We have 
* already seen the huge difference between the across-trial interaction from 
* the meta-regression and the pooled interaction from the model above. However,
* in the one-stage model above, the across-trial interaction is still 
* contributing to the pooled interaction, as we have not separated out the 
* ecological bias.

* In the lectures, we discussed the need to separate out interactions by
* centring covariates. Let us calculate a new sex variable, centred by the 
...
* and the within-trial interaction (the ecological bias removed by centring).
...
* Better still, we can also utilise the wonderful 'ipdmetan'.
...
* variances and adjustment terms, but the one-stage does not.
* This leads to differences in the final meta-analysis results.
