...
# Introduction
  ...
  Meta-analysis (MA) is a statistical method that involves combining information from multiple studies. While initially, meta-analyses were limited in aggregated data (AD) in the early 1990s individual participant data meta-analysis (IPD-MA) was introduced [@CHALMERS_1993]. In IPD-MA the participant level information is available and therefore evidence from multiple studies can be analysed centrally. Collecting the IPD may be a difficult and time consuming task, but nonetheless IPD-MA is considered the gold standard in evidence synthesis [@Stewart_1993 ; @Stewart_1995 ;@Stewart_2002] and offers great opportunities [@van_Walraven_2010] that in AD-MA are considered impossible. Besides when investigating overall treatment effects where AD-MA and IPD-MA may be mathematically equivalent, IPD-MA offers (1) the possibility to standardize subgroup definitions and outcomes across studies, (2) higher validity and credibility of subgroup findings, (3) increased flexibility to search for subgroups based on combinations of patient and/or disease characteristics (4) the possibility to avoid ecological BIAS (5) model more complicated than linear functional forms (6) better prediction model training and (7) synthesizing evidence. 

**Reporting issues**
  IPD-MA may be conducted either in one stage or two stages. In one-stage IPD-MA, an appropriate statistical model is applied and IPD from every study are analysed simultaneously, whilst accounting for within-studies clustering of the participants. On the other hand, in two-stage IPD-MA a statistical model of choice is fitted per study. Subsequently the estimates extracted are pooled using inverse-variance meta-analytical methods. Both approaches have a variety of parameters and results that should be reported in the abstract, the methods and the results section. An extented version of PRISMA for IPD[@Stewart_2015].....
For instance, in one-stage IPD-MA the multi-level model may include either fixed, random or stratified terms. Furthermore, the parameters may be estimated with maximum likelihood or restricted maximum likelihood and their confidence intervals may be estimated by using approximations either based on a z-distribution or t‐distribution. On the other hand, in two-stage IPD-MA 1) heterogeneity measures ( $I^2$ , Cochran's Q $\tau^2$) 2) $\tau$ estimator methods (Empirical Bayes, REML, ML, DerSimonian laird) 3) forest plots (if applicable) and 4) prediction intervals of the estimates [@IntHout_2016] should be included according to the PRISMA-IPD statement . 

**Subgroup Analysis**
  When the goal of the IPD-MA is to detect treatment effect modification a variety of methods is available. Simmonds et al. [@Simmonds_2015] showed that meta-regression, per-subgroup meta-analysis, meta-analysis of interaction terms and one-stage IPD-MA were typically used.   Simmnond and Higgins [@Simmonds_2007] mathematically proved that one-stage IPD-MA are always more powerful than meta-analysis of interaction terms and meta-regression. Nevertheless, the assumptions made were unrealistic. Fisher et al. [@Fisher_2011] critically reviewed one-stage IPD-MA of them on how to detect patient level effect modification in IPD-MA. They concluded that one-stage IPD-MA (mixed-effects modelling) allows for more complex analysis, but is more difficult to perform than pooling within-trial interaction terms. Subsequently, Hua et al. [@Hua_2016] suggested that these mixed effects models should also centre the effect modifiers to their mean, in order to separate across and within trial information. 
