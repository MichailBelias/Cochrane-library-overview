...
We identified 1538 published IPD-MAs. We excluded 60 duplicates.
...
pander(IPD_Table_Medical_Freq, style = "grid",keep.line.breaks = TRUE, caption = "Individual participant meta-analysis per medical field", )
...
The articles that are tagged as meta-analyses of individual participant data are increasing per year.
...
gghistogram(IPD_MA, x =  "Number.of..Patients", y = "..count..",bins = 50, palette = "jco", 
            ggtheme = theme_minimal(), title = "Number of participants included per meta-analysis", add = "median", xlab = "Participants per Meta-analysis" )  + annotate(geom="text", x=15, y=20, label= paste("median:", median(IPD_MA$Number.of..Patients,na.rm = T)), color="red") + theme(plot.title = element_text(hjust = 0.5,size = 18, face = "bold.italic")) +  xlim(0,25000)
...
IPD_Outcome_frequencies =  unlist(strsplit(gsub("[\r\n]" ,"",IPD_MA$Type.of.primary.outcome),";"))
IPD_Outcome_frequencies =  gsub(" ", "",IPD_Outcome_frequencies )
IPD_Outcome_frequencies =  table(IPD_Outcome_frequencies)


pander( IPD_Outcome_frequencies[names(IPD_Outcome_frequencies) %in% c("Binary", "Continuous", "Time-to-event")], 
        caption = "Table 2. Type of outcomes investigated",style= "grid")
```
...
```{r}
IPD_Outcome_frequencies =  unlist(strsplit(gsub("[\r\n]" ,"  ",IPD_MA$Effect.estimate),";"))
IPD_Outcome_frequencies =  trimws(IPD_Outcome_frequencies )
table(IPD_Outcome_frequencies) 
